...
def test_numbers_are_sequential_within_a_project(session):
    ...
    numbers = [
        make_task(session, project_id=project.id, title=f"t{i}", created_by=owner.id).number
        for i in range(5)
    ]
    session.flush()
...
def test_numbers_are_not_recycled_after_delete(session):
    ...
    t1 = make_task(session, project_id=project.id, title="t1", created_by=owner.id)
    t2 = make_task(session, project_id=project.id, title="t2", created_by=owner.id)
    session.flush()
    session.delete(t2)
    session.flush()

    t3 = make_task(session, project_id=project.id, title="t3", created_by=owner.id)
    session.flush()
